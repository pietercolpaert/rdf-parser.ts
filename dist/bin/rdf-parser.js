...
var CoreParser = class {
  ...
  constructor(input2, options, callbacks, state) {
    ...
    const format2 = (options.format ?? "").toLowerCase();
  ...
  parseSubject(graph) {
    ...
    if (term.termType === "Literal" || term.termType === "DefaultGraph" || term.termType === "Variable" || (this.strictNTriples || this.strictNQuads) && !this.allowLegacyTripleTerms && term.termType === "Quad") {
...
function resolveIri(value, baseIRI2) {
  if (!baseIRI2 || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(value)) return value;
  try {
    return new URL(value, baseIRI2).href;
  } catch {
    return value;
  }
}
